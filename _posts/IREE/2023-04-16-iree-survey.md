...
[IREE](https://github.com/iree-org/iree) (Intermediate Representation Execution Environment)是一种基于MLIR的端到端编译器，可以将ML模型lower到统一的IR。
...
![workflow](/assets/img/blog/img_iree_survey/iree_workflow.png)

# Main Dialect

背景知识介绍：The main tasks of a runtime are to manage resources and schedule execution

[MLIR dialects - IREE](https://iree.dev/reference/mlir-dialects/)
...
![compilation](/assets/img/blog/img_iree_survey/Untitled.png)

# 编译流程

IREE目前支持将 `MHLO` 或 `XLA、Torch Tensor` 和 `TOSA` 作为输入，经过一系列passes编译生成IREE定义的 `VM bytecode` 中间产物，其中硬件相关代码会编译成相应的`Executable`，保存在VM bytecode中供host进行调用。
...
![HAL](/assets/img/blog/img_iree_survey/Untitled%202.png)
...
源码位于 [compiler\src\iree\compiler\Bindings\Native\Transforms\Passes.cpp](https://github.com/iree-org/iree/blob/main/compiler/src/iree/compiler/Bindings/Native/Transforms/Passes.cpp#L38-L46)，最主要的函数是 `buildTransformPassPipeline`
...
位于 compiler\src\iree\compiler\Dialect\Flow\Transforms\Passes.cpp ，最主要的函数是 [buildFlowTransformPassPipeline](https://github.com/iree-org/iree/blob/main/compiler/src/iree/compiler/Dialect/Flow/Transforms/Passes.cpp#L182-L388)
...
位于 compiler\src\iree\compiler\Dialect\Stream\Transforms\Passes.cpp，最主要的函数是 [buildStreamTransformPassPipeline](https://github.com/iree-org/iree/blob/main/compiler/src/iree/compiler/Dialect/Stream/Transforms/Passes.cpp#L297-L335)
...
位于 compiler\src\iree\compiler\Dialect\HAL\Transforms\Passes.cpp，最主要的函数是 [buildHALTransformPassPipeline](https://github.com/iree-org/iree/blob/main/compiler/src/iree/compiler/Dialect/HAL/Transforms/Passes.cpp#L209-L376)
