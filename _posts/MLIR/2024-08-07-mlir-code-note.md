...
![value的use-chain](/assets/img/blog/img_mlir_note/Untitled.png)
...
1.isa : 不能在cast之后使用isa，有类似场景请用dyn_cast

2.isa_and_nonnull / isa_and_present：允许op为null作为输入，返回null
...
3.dyn_cast ： 尝试转换，失败时返回null，op为null时报错

4.dyn_cast_if_present / dyn_cast_or_null ：尝试转换，失败时返回null，op为null时返回null
...
1.llvm:function_ref 定义inline func，用于传递函数指针
...
![op,res,operand](/assets/img/blog/img_mlir_note/Untitled.png)
...
![op addr](/assets/img/blog/img_mlir_note/Untitled%202.png)
...
- 然后在 `xxxDialectOps.cpp` 中写 build 等方法
...
见 Interface 节中 `MemoryEffectOpInterface` 相关用法
